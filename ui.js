/* =========================================================
   Fichier : js/ui.js
   Rôle : Gestion de l'interface, des panneaux, et des graphiques
   ========================================================= */
...
window.closeRightSidebar = function() {
    ...
    if (window.webcamLayerGroup) {
        window.webcamLayerGroup.eachLayer(m => m.dragging.disable());
    }
};
...
window.toggleMultiSelect = function() {
    ...
    if (window.isMultiSelect) {
        btn.innerText = "🔲 Sélec. Multiple : ON";
        btn.style.background = "#3498db";
    } else {
        btn.innerText = "🔲 Sélec. Multiple : OFF";
...
window.initSidebarResizer = function() {
    ...
    rs.addEventListener('mousedown', () => {
        isR = true; rs.classList.add('active');
    ...
    document.addEventListener('mouseup', () => {
        if (isR) {
            isR = false; rs.classList.remove('active');
...
// ---------------------------------------------------------
// 2. MODALES SPÉCIFIQUES (Météo, Webcams)
// ---------------------------------------------------------
...
window.openMeteoWidget = function() {
    ...
    document.getElementById('right-title').innerText = "⛅ Météo Live";
    ...
    window.switchMeteoTab = function(p) {
        let c = document.getElementById('meteo-content');
        if (!c) return;
        document.querySelectorAll('.meteo-tab-btn').forEach(b => b.style.opacity = '0.5');
        document.getElementById('btn-meteo-' + p).style.opacity = '1';
        
        if (p === 'meteoblue') c.innerHTML = `<iframe src="https://www.meteoblue.com/fr/meteo/widget/three/42.767N0.582E1791_Europe%2FParis?geoloc=fixed&nocurrent=0&noforecast=0&days=3&temp=1&layout=light" frameborder="0" style="width:100%;height:100%;"></iframe>`;
        else if (p === 'windy') c.innerHTML = `<iframe src="https://embed.windy.com/embed2.html?lat=42.767&lon=0.582&zoom=12&overlay=wind&product=ecmwf" frameborder="0" style="width:100%;height:100%;"></iframe>`;
        else if (p === 'ventusky') c.innerHTML = `<iframe src="https://www.ventusky.com/?p=42.767;0.582;12&l=snow" frameborder="0" style="width:100%;height:100%;"></iframe>`;
    };
    
    sl.innerHTML = `
        <div style="display:flex;gap:5px;margin-bottom:15px;">
            <button id="btn-meteo-meteoblue" class="meteo-tab-btn" onclick="window.switchMeteoTab('meteoblue')" style="flex:1;background:#3498db;color:white;border:none;padding:10px;border-radius:4px;cursor:pointer;opacity:1;">Meteoblue</button>
            <button id="btn-meteo-windy" class="meteo-tab-btn" onclick="window.switchMeteoTab('windy')" style="flex:1;background:#e67e22;color:white;border:none;padding:10px;border-radius:4px;cursor:pointer;opacity:0.5;">Windy</button>
            <button id="btn-meteo-ventusky" class="meteo-tab-btn" onclick="window.switchMeteoTab('ventusky')" style="flex:1;background:#2c3e50;color:white;border:none;padding:10px;border-radius:4px;cursor:pointer;opacity:0.5;">Ventusky</button>
        </div>
        <div id="meteo-content" style="background:#fff;border-radius:6px;border:1px solid #eee;padding:10px;height:calc(100vh - 200px);">
            <iframe src="https://www.meteoblue.com/fr/meteo/widget/three/42.767N0.582E1791_Europe%2FParis?geoloc=fixed&nocurrent=0&noforecast=0&days=3&temp=1&layout=light" frameborder="0" style="width:100%;height:100%;"></iframe>
        </div>`;
    sb.classList.add('open');
};
...
window.openWebcamGrid = function() {
    ...
    document.getElementById('right-title').innerText = "📷 Webcams";
    document.getElementById('multi-tools').style.display = 'none';
    document.getElementById('view-edit').style.display = 'none';
    document.getElementById('view-selection').style.display = 'block';
    ...
    if (window.webcamsData.length === 0) html += "<p>Aucune caméra configurée.</p>";
        ...
        window.webcamsData.forEach(w => {
            let isImg = w.url.toLowerCase().match(/\.(jpeg|jpg|png|gif)$/) != null;
            let mHtml = isImg ? `<img src="${w.url}" style="width:100%;border-radius:4px;"/>` : `<div style="width:100%;height:300px;border-radius:4px;overflow:hidden;"><iframe src="${w.url}" style="width:100%;height:100%;border:none;"></iframe></div>`;
            html += `
                <div style="background:#f8f9fa;padding:15px;border-radius:6px;border:1px solid #ddd;">
                    <b>${w.icon} ${w.nom}</b><br><br>${mHtml}<br>
                    <button onclick="window.enableWebcamEdit('${w.nom.replace(/'/g,"\\'")}')" style="width:100%;background:#f39c12;color:white;border:none;padding:8px;border-radius:4px;margin-top:10px;">✏️ Éditer Position</button>
                </div>`;
        });
    }
    html += "</div>";
...
window.openSingleWebcam = function(nom) {
    let wc = window.webcamsData.find(w => w.nom === nom);
    if (!wc) return;
    
    let sb = document.getElementById('right-sidebar');
    let sl = document.getElementById('selected-list');
    
    document.getElementById('right-title').innerText = `${wc.icon} ${wc.nom}`;
    document.getElementById('multi-tools').style.display = 'none';
    document.getElementById('view-edit').style.display = 'none';
    document.getElementById('view-selection').style.display = 'block';
    
    let isImg = wc.url.toLowerCase().match(/\.(jpeg|jpg|png|gif)$/) != null;
    let mHtml = isImg ? `<img src="${wc.url}" style="width:100%;border-radius:4px;"/>` : `<div style="width:100%;height:300px;border-radius:4px;overflow:hidden;"><iframe src="${wc.url}" style="width:100%;height:100%;border:none;"></iframe></div>`;
    
    sl.innerHTML = `
        <div style="background:#f8f9fa;padding:15px;border-radius:6px;">
            ${mHtml}
            <button onclick="window.enableWebcamEdit('${wc.nom.replace(/'/g,"\\'")}')" style="margin-top:15px;width:100%;background:#f39c12;color:white;border:none;padding:10px;border-radius:4px;">✏️ Éditer position</button>
        </div>`;
    sb.classList.add('open');
};

window.enableWebcamEdit = function(nom) {
    window.webcamLayerGroup.eachLayer(m => {
        if (m.wcData.nom === nom) {
            m.dragging.enable();
            let sl = document.getElementById('selected-list');
            sl.innerHTML = `
                <div style="padding:15px;background:#fff3cd;border:1px solid #ffeeba;border-radius:6px;">
                    <b>Mode Édition : ${nom}</b><br>
                    1. Glissez le marqueur sur la carte<br>
                    2. Emoji de la caméra :<br>
                    <input type="text" id="edit-wc-icon" value="${m.wcData.icon}" style="width:50px;text-align:center;"><br><br>
                    <div style="display:flex;gap:10px;">
                        <button onclick="window.openSingleWebcam('${nom.replace(/'/g,"\\'")}')" style="flex:1;padding:8px;">Annuler</button>
                        <button onclick="window.saveWebcamEdit('${nom.replace(/'/g,"\\'")}',this)" style="flex:2;background:#27ae60;color:white;border:none;">✅ Sauver</button>
                    </div>
                </div>`;
        }
    });
...
// ---------------------------------------------------------
// 3. EDITION DES CANONS ET DES BONS D'INTERVENTION (TICKETS)
// ---------------------------------------------------------

window.startEdit = function(id) {
    window.currentlyEditingId = id;
    let l = window.selectedCannonsMap.get(id);
    let p = l.feature.properties;
    
    document.getElementById('edit-id-display').innerText = p.id;
    document.getElementById('edit-id').value = p.id;
    document.getElementById('edit-lat').value = l.getLatLng().lat.toFixed(7);
    document.getElementById('edit-lng').value = l.getLatLng().lng.toFixed(7);
    
    let fc = document.getElementById('dynamic-edit-fields');
    fc.innerHTML = "";
    p.fullData.forEach(i => {
        if (i.k.toLowerCase().match(/(regard|xy|gps)/)) return;
        fc.innerHTML += `<div style="margin-bottom:10px;"><div class="edit-label">${i.k}</div><input type="text" class="edit-input dyn-field" data-key="${i.k}" value="${i.v}"></div>`;
    });
    
    document.getElementById('view-selection').style.display = 'none';
    document.getElementById('view-edit').style.display = 'block';
    document.getElementById('right-title').innerText = "✏️ Édition";
    window.updateMap();
};
...
window.editFullTicket = function(canonId, defaut) {
    let safeDef = defaut.replace(/[^a-zA-Z0-9]/g, ""); 
    let idDiv = `ticket-edit-${canonId}-${safeDef}`; 
    let el = document.getElementById(idDiv); 
    
    if(!el) { idDiv = `t-${canonId}-${safeDef}`; el = document.getElementById(idDiv); }
    if(!el) { alert("Erreur : Panneau introuvable."); return; }
    ...
    ticket.fullData.forEach(d => {
        ...
        if (kLow.includes('regard') || kLow === 'id' || kLow.includes('n°')) { 
            html += `<div style="margin-bottom:8px;"><div class="edit-label">${d.k}</div><input type="text" class="edit-input tick-dyn-field-${idDiv}" data-key="${d.k}" value="${valStr}" readonly style="background:#e1e8ed; color:#7f8c8d; margin-bottom:0; cursor:not-allowed;"></div>`; 
        } 
        else if (kLow.includes('etat') || kLow.includes('état') || kLow === 'statut') { 
            let statuts = ['à faire', 'en cours', 'fait']; 
            let options = statuts.map(s => `<option value="${s}" ${valStr.toLowerCase().includes(s) ? 'selected' : ''}>${s.toUpperCase()}</option>`).join(''); 
            html += `<div style="margin-bottom:8px;"><div class="edit-label">${d.k}</div><select class="edit-input tick-dyn-field-${idDiv}" data-key="${d.k}" style="margin-bottom:0;">${options}</select></div>`; 
        } 
        else if (valStr.length > 40 || kLow.includes('cause') || kLow.includes('intervention') || kLow.includes('commentaire') || kLow.includes('action')) { 
            html += `<div style="margin-bottom:8px;"><div class="edit-label">${d.k}</div><textarea class="edit-input tick-dyn-field-${idDiv}" data-key="${d.k}" style="margin-bottom:0;" rows="2">${valStr}</textarea></div>`; 
        } 
        else { 
            html += `<div style="margin-bottom:8px;"><div class="edit-label">${d.k}</div><input type="text" class="edit-input tick-dyn-field-${idDiv}" data-key="${d.k}" value="${valStr}" style="margin-bottom:0;"></div>`; 
        }
    });
    
    html += `<div style="display:flex; gap:10px; margin-top:15px;">
        <button onclick="window.renderSelectionPanel()" style="flex:1; padding:10px; background:#bdc3c7; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold;">Annuler</button>
        <button onclick="window.prepareTicketSave('${canonId}', '${defaut.replace(/'/g, "\\'")}', '${idDiv}')" style="flex:2; padding:10px; background:#2ecc71; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold;">Suivant ➔</button>
    </div></div>`; 
...
window.prepareTicketSave = function(canonId, defaut, idDiv) {
    ...
    document.querySelectorAll(`.tick-dyn-field-${idDiv}`).forEach(inp => { 
        let k = inp.getAttribute('data-key'); let v = inp.value; 
        if(!k.toLowerCase().includes('regard') && k.toLowerCase() !== 'id') { fieldsObj[k] = v; } 
        if(k.toLowerCase().includes('etat') || k.toLowerCase().includes('état') || k.toLowerCase() === 'statut') { newStatutForLocal = v; } 
    });
    ...
    ticket.fullData.forEach(d => { 
        let oldValRaw = d.v.toString().trim(); 
        let newValRaw = (fieldsObj[d.k] !== undefined) ? fieldsObj[d.k].trim() : oldValRaw; 
        if (fieldsObj[d.k] !== undefined && newValRaw !== oldValRaw) { 
            hasChanges = true; 
            let oldDisplay = oldValRaw === "" ? "<i>(vide)</i>" : oldValRaw; 
            let newDisplay = newValRaw === "" ? "<i>(vide)</i>" : newValRaw; 
    ...
    if (!hasChanges) { alert("Aucune modification."); window.renderSelectionPanel(); return; }
    ...
    if(el) { 
        el.innerHTML = `
            <div style="background:#fff3cd; padding:15px; border-radius:6px; border:2px solid #f39c12; margin-top:10px;">
                <div style="font-size:14px; font-weight:bold; color:#d35400; margin-bottom:15px; text-transform:uppercase;">⚠️ Vérification :</div>
                ${diffHtml}
                <div style="display:flex; gap:10px; margin-top:20px;">
                    <button onclick="window.renderSelectionPanel()" style="flex:1; padding:10px; background:#bdc3c7; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold;">Annuler</button>
                    <button onclick="window.confirmSaveTicket('${canonId}', '${defaut.replace(/'/g, "\\'")}', '${idDiv}')" style="flex:2; padding:10px; background:#27ae60; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold;">Envoyer 🚀</button>
                </div>
            </div>`; 
    }
};
...
window.renderSelectionPanel = function() {
    ...
    document.getElementById('view-selection').style.display = 'block'; 
    document.getElementById('sel-count').innerText = window.selectedCannonsMap.size; 
    ...
    if (window.mainTheme === 'suivi_entretien') {
        listDiv.innerHTML = `<button onclick="window.saveMultiSuivi(this)" style="width:100%; background:#27ae60; color:white; border:none; padding:12px; border-radius:6px; cursor:pointer; font-weight:bold; margin-bottom:15px; font-size:14px; box-shadow:0 2px 5px rgba(0,0,0,0.2);">💾 Sauvegarder la sélection</button>`;
    }
    ...
    window.selectedCannonsMap.forEach((layer, id) => {
        ...
        let active = window.getActiveTickets(p.id); 
        let history = window.getHistoryTickets(p.id); 
        ...
        if (window.mainTheme === 'suivi_entretien') {
            ...
            let isPrev = s[3].toString().toLowerCase().trim() === "oui";
            let rem = s[5] || "";
            let bY = window.getV(p, "année de buse") || window.getV(p, "annee buse") || "Inconnue";
            let vAdY = window.getV(p, "année vanne ad") || window.getV(p, "annee vanne ad") || "Inconnue";
            let vY = window.getV(p, "année vanne") || window.getV(p, "annee vanne") || "Inconnue";
            
            mainContent = `
                <div style="background:#eafaf1; padding:10px; border-radius:6px; border:1px solid #2ecc71; margin-bottom:10px; font-size:12px;">
                    <h4 style="margin:0 0 8px 0; color:#27ae60;">🛠️ Suivi Entretien</h4>
                    <div style="display:flex; gap:10px; margin-bottom:8px;">
                        <label style="cursor:pointer;"><input type="checkbox" id="chk-dec-single-${p.id}" ${isDec?'checked':''}> Déconnexion</label>
                        <label style="cursor:pointer;"><input type="checkbox" id="chk-prev-single-${p.id}" ${isPrev?'checked':''}> Préventif Usine</label>
                    </div>
                    <input type="text" id="rem-single-${p.id}" value="${rem}" placeholder="Remarque..." style="width:100%; box-sizing:border-box; padding:6px; border:1px solid #ccc; border-radius:4px;">
                </div>
                <div style="background:#fdfdfe; padding:10px; border-radius:6px; border:1px solid #ccc; font-size:12px;">
                    <h4 style="margin:0 0 8px 0; color:#2c3e50;">📊 Données Techniques</h4>
                    <b>Modèle :</b> ${window.getV(p, window.keyType)}<br><b>Année Buse :</b> ${bY}<br><b>Année Vanne AD :</b> ${vAdY}<br><b>Année Vanne :</b> ${vY}
                </div>
                ${histHtml}`;
        } 
        // --- THEME HISTORIQUE ---
        ...
        // --- THEME DEFAUTS ---
            ...
            if (active.length === 0) { 
                ...
                mainContent = active.map(t => {
                    ...
                    let safeDef = t.defaut.replace(/'/g, "\\'"); 
        ...
        let editBtn = '';
        if (!window.isMultiSelect) {
            if (window.mainTheme === 'type') editBtn = `<button class="btn-action-primary" onclick="window.startEdit('${p.id}')">✏️ Éditer ce canon</button>`;
            else if (window.mainTheme === 'suivi_entretien') editBtn = `<button class="btn-action-primary" style="background:#f39c12; margin-top:10px;" onclick="window.startEdit('${p.id}')">⚙️ Éditer les Données Techniques</button>`;
        }
...
window.triggerGlobalFilterChart = function() {
    ...
    const centerTextPlugin = {
        ...
        beforeDraw: function(chart) {
            ...
            let width = chart.width, height = chart.height, ctx = chart.ctx; ctx.restore();
    ...
    if (window.mainTheme === 'defauts') {
        ...
        visibleCannons.forEach(f => {
            ...
            if(acts.length === 0) { cRas++; } 
                ...
                acts.forEach(t => { 
                    if(t.statut.includes('faire')) cUrgent++; else cEnCours++; 
                    activeList.push({id: p.id, piste: window.getV(p, window.keyPiste), ticket: t}); 
                }); 
            }
        });

        chartLabels = ['À faire', 'En cours', 'RAS']; 
        chartData = [cUrgent, cEnCours, cRas]; 
        chartColors = ['#e74c3c', '#f39c12', '#bdc3c7']; 
        chartType = 'doughnut';
        
        let total = cUrgent + cEnCours + cRas; 
        let faultPerc = total > 0 ? (((cUrgent + cEnCours) / total) * 100).toFixed(1) : 0;
        ...
        let pCounts = {}; activeList.forEach(item => { pCounts[item.piste] = (pCounts[item.piste] || 0) + 1; });
        ...
        if (activeList.length === 0) { 
            html += `<div style="font-size:12px; color:#7f8c8d; text-align:center; padding:10px;">Aucun défaut sur les canons affichés.</div>`; 
        } else {
        ...
        setTimeout(() => {
            ...
            if(cPiste){
                ...
                window.chartInstances['gfc-piste'] = new Chart(cPiste.getContext('2d'), { type: 'bar', data: { labels: pLabels, datasets: [{ data: pData, backgroundColor: '#e74c3c', borderRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: {font:{size:9}} } } } });
            }
        }, 100);
    } 
    // --- AUTRES THEMES (TYPE, REPETEUR, TRANSFO) ---
        ...
        let counts = {}; visibleCannons.forEach(f => { let v = window.getV(f.properties, key); counts[v] = (counts[v] || 0) + 1; });
        chartLabels = Object.keys(counts).sort(); chartData = chartLabels.map(k => counts[k]);
        
        if (window.mainTheme === 'type') { 
            chartColors = chartLabels.map(k => { 
                let kL = k.toLowerCase(); 
                if(["demak lenko", "demac lenko"].includes(kL)) return '#0000FF'; 
                if(["sufag", "street", "taurus", "taurax", "peak"].includes(kL)) return '#008000'; 
                return window.technoColors[kL] || '#FFD700'; 
            }); 
        } 
    ...
    // --- THEME HISTORIQUE ---
        ...
        let s = window.saisonsList[window.histSeasonIdx]; 
        let metricName = window.histMetric === 'eau' ? 'Eau (m³)' : (window.histMetric === 'air' ? 'Air' : 'Temps (h)');
        ...
        if (isSinglePiste || visibleCannons.length <= 40) {
            ...
            chartLabels = sorted.map(c => c.properties.id); 
            chartData = sorted.map(c => c.properties.historique[s] ? c.properties.historique[s][window.histMetric] : 0); 
            chartColors = '#3498db'; 
            let pisteName = isSinglePiste ? window.currentPistes[0] : "Canons";
            ...
            let pisteTotals = {}; 
            visibleCannons.forEach(f => { 
                let p = window.getV(f.properties, window.keyPiste); 
                let val = f.properties.historique[s] ? f.properties.historique[s][window.histMetric] : 0; 
                pisteTotals[p] = (pisteTotals[p] || 0) + val; 
            });
    ...
    // --- THEME SUIVI ENTRETIEN ---
        ...
        visibleCannons.forEach(f => {
            let p = f.properties; let s = window.suiviEntretienData.find(row => row[0] == p.id) || [];
            let hasFault = window.getActiveTickets(p.id).length > 0;
            let isDec = s[2] && s[2].toString().toLowerCase().trim() === "oui";
            let isPrev = s[3] && s[3].toString().toLowerCase().trim() === "oui";
            
            if(hasFault) cDef++; else if(isPrev) cPrev++; else if(isDec) cDec++; else cOk++;

            let bY = window.getV(p, "année de buse") || window.getV(p, "annee buse");
            let vY = window.getV(p, "année vanne") || window.getV(p, "annee vanne");
            if(bY && bY !== "N/A") bYearCounts[bY] = (bYearCounts[bY]||0)+1;
            if(vY && vY !== "N/A") vYearCounts[vY] = (vYearCounts[vY]||0)+1;
        });
        
        let topBuse = Object.entries(bYearCounts).sort((a,b)=>b[1]-a[1]).slice(0,3).map(x=>`${x[0]} <span style="color:#7f8c8d">(${x[1]})</span>`).join('<br>');
        let topVanne = Object.entries(vYearCounts).sort((a,b)=>b[1]-a[1]).slice(0,3).map(x=>`${x[0]} <span style="color:#7f8c8d">(${x[1]})</span>`).join('<br>');

        html += `
            <div style="display:flex; gap:10px; text-align:center; margin-bottom:15px;">
                <div style="flex:1; background:#fff; border:1px solid #ccc; border-radius:6px; padding:10px;"><b style="font-size:20px; color:#27ae60;">${cDec}</b><br><span style="font-size:10px;">Déconnectés</span></div>
                <div style="flex:1; background:#fff; border:1px solid #ccc; border-radius:6px; padding:10px;"><b style="font-size:20px; color:#9b59b6;">${cPrev}</b><br><span style="font-size:10px;">Préventifs</span></div>
            </div>
            <div style="display:flex; gap:10px; margin-bottom:15px; font-size:11px;">
                <div style="flex:1; background:#fdfdfe; border:1px solid #e1e8ed; padding:10px; border-radius:6px;"><h4 style="margin:0 0 5px 0; color:#3498db;">Top Années Buse</h4>${topBuse || "Aucune info"}</div>
                <div style="flex:1; background:#fdfdfe; border:1px solid #e1e8ed; padding:10px; border-radius:6px;"><h4 style="margin:0 0 5px 0; color:#e67e22;">Top Années Vanne</h4>${topVanne || "Aucune info"}</div>
            </div>
            <div class="piste-chart-container"><div style="text-align:center; font-weight:bold; font-size:12px; margin-bottom:10px;">Statuts Principaux (Priorité Haute)</div><div style="position:relative; height:180px; width:100%;"><canvas id="suivi-pie-global"></canvas></div></div>`;

        let pistes = Array.from(window.columnValuesSets[window.keyPiste] || []).sort();
        let selectHtml = `
            <div style="background:#f8f9fa; padding:15px; border-radius:6px; border:1px solid #ddd; margin-top:15px;">
                <div class="edit-label">Sélectionner une piste pour l'entretien :</div>
                <select class="edit-input" style="margin-bottom:0;" onchange="window.renderSuiviList(this.value)">
                    <option value="">-- Choisir une piste --</option>
                    ${pistes.map(p => `<option value="${p}">${p}</option>`).join('')}
                </select>
            </div>
            <div id="suivi-canons-container"></div>
        `;
        
        document.getElementById('multi-tools').style.display = 'flex'; 
        document.getElementById('view-selection').style.display = 'block'; 
        document.getElementById('view-edit').style.display = 'none'; 
        document.getElementById('selected-list').innerHTML = html + selectHtml; 
        
        setTimeout(() => {
            if(window.chartInstances['suivi-pie-global']) window.chartInstances['suivi-pie-global'].destroy();
            let canvas = document.getElementById('suivi-pie-global');
            if (canvas) window.chartInstances['suivi-pie-global'] = new Chart(canvas.getContext('2d'), { type: 'doughnut', data: { labels: ['En Défaut', 'Préventif Usine', 'Déconnecté', 'Connecté (⚡)'], datasets: [{ data: [cDef, cPrev, cDec, cOk], backgroundColor: ['#e74c3c', '#9b59b6', '#27ae60', '#f1c40f'] }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels:{boxWidth:10, font:{size:10}} } } } });
        }, 100);
        return; 
    }
