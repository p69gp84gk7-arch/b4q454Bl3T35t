...
window.finalizeDraw = () => {
    ...
    const drawObj = { id: Date.now(), type, name: type==='area'?'Surface':'Tracé Ligne', ptsGPS: [...currentPoints], visible: true, color, weight: 4, isEditing: false, editGroup: L.layerGroup().addTo(map) };
...
window.finalizeCircle = (center, radius) => {
    const drawObj = { id: Date.now(), type: 'circle', name: 'Cercle/Rayon', center, radius, visible: true, color: '#9b59b6', weight: 3, isEditing: false, editGroup: L.layerGroup().addTo(map) };
...
function recalculateStats(d) {
    ...
    // Mise à jour de l'étiquette si elle est à l'écran (droite OU gauche)
...
function makeEditable(d, isProj, pid) {
    ...
    d.ptsGPS.forEach((pt, idx) => {
        ...
        m.on('drag', (e) => {
            ...
            // Update l'altitude live si elle est sur "Auto"
...
// Fonction pour générer le tableau d'édition (partagé entre droite et gauche)
function generateEditorTable(d, isProj) {
    if(!d.isEditing || d.type === 'circle') return '';
    let editorHtml = `<div style="background:#111; padding:5px; margin-top:5px; border-radius:3px; font-size:11px;">
    ...
    d.ptsGPS.forEach((pt, i) => {
        ...
        let zActuel = pt.customZ !== undefined ? pt.customZ : getZ(l);
        let zVal = pt.customZ !== undefined ? pt.customZ.toFixed(2) : '';
        let zPlace = zActuel !== null ? zActuel.toFixed(2) : 'Auto';
        
        editorHtml += `<tr>
            <td style="color:#e74c3c; font-weight:bold;">${i+1}</td>
            <td><input id="edit-x-${d.id}-${i}" value="${l[0].toFixed(2)}" oninput="applyPointEdits(${d.id}, ${isProj})" style="width:65px; background:#222; color:#fff; border:1px solid #555; text-align:center;"></td>
            <td><input id="edit-y-${d.id}-${i}" value="${l[1].toFixed(2)}" oninput="applyPointEdits(${d.id}, ${isProj})" style="width:65px; background:#222; color:#fff; border:1px solid #555; text-align:center;"></td>
            <td><input id="edit-z-${d.id}-${i}" value="${zVal}" placeholder="${zPlace}" onchange="applyPointEdits(${d.id}, ${isProj})" title="Laissez vide pour l'altitude du terrain" style="width:45px; background:#2980b9; color:#fff; border:1px solid #555; text-align:center;"></td>
        </tr>`;
    });
    return editorHtml + `</table></div>`;
}
...
function updateDrawUI() {
    ...
    drawStore.forEach(d => {
        let btns = d.type === 'line' ? 
            `<button onclick="generateProfileById(${d.id})" style="width:100%; margin-top:5px; background:#333; color:#fff; border:1px solid #555; padding:5px; cursor:pointer; font-weight:bold; border-radius:3px;">📏 Afficher Profil Altimétrique</button>` : 
            `<div style="display:flex; gap:3px; margin-top:5px; flex-wrap:wrap;">
                <button onclick="calculateVolume(${d.id}, 'hollow')" style="flex:1; font-size:0.75em; background:#2980b9; color:#fff; border:none; cursor:pointer; padding:5px; border-radius:3px;">📉 Calcul Déblai</button>
                <button onclick="calculateVolume(${d.id}, 'mound')" style="flex:1; font-size:0.75em; background:#e67e22; color:#fff; border:none; cursor:pointer; padding:5px; border-radius:3px;">📈 Calcul Remblai</button>
                <button onclick="calculateVolume(${d.id}, 'slope')" style="flex:1; font-size:0.75em; background:#8e44ad; color:#fff; border:none; cursor:pointer; padding:5px; border-radius:3px;">📐 Vol. Courbe</button>
                <button onclick="calculateVolume(${d.id}, 'plane')" style="flex:1; font-size:0.75em; background:#9b59b6; color:#fff; border:none; cursor:pointer; padding:5px; border-radius:3px;">📏 Vol. Plan</button>
                <button onclick="generate3DView(${d.id})" style="flex:1; min-width:100%; font-size:0.8em; font-weight:bold; background:#34495e; color:#fff; border:1px solid #555; cursor:pointer; padding:5px; margin-top:2px; border-radius:3px;">👁️ Lancer Vue 3D</button>
            </div>`;
...
window.calculateVolume = (id, type) => {
    ...
    setTimeout(() => {
        ...
        recalculateStats(d); 
        updateDrawUI(); updateProjectUI(); // Force le rafraîchissement
    }, 50);
};
...
// ==========================================
// 7. VUE 3D PARFAITE (SUIVI & CALQUES)
// ==========================================
        ...
        try {
            ...
            for (let j = 0; j < yV.length; j++) {
                ...
                for (let i = 0; i < xV.length; i++) {
                    ...
                    if (isPointInPolygon([x, y], l93)) { 
                        const zVal = getZ([x, y]);
                        rT.push(zVal); rB.push(aR*x + bR*y + cR); 
                        if(zVal !== null) hasData = true;
                    } else { rT.push(null); rB.push(null); }
                } zT.push(rT); zB.push(rB);
            }
            
            if(!hasData) { document.getElementById('plot-3d').innerHTML = `<h3 style="color:#e74c3c; text-align:center; margin-top:20%;">Surface trop petite.</h3>`; return; }
            ...
            const tracePoints = {
                ...
                marker: { color: '#e74c3c', size: 5, symbol: 'circle' }
            };
            ...
            }).catch(e => { document.getElementById('plot-3d').innerHTML = `<h3 style="color:#e74c3c; text-align:center;">Erreur 3D</h3>`; });
        } catch (err) { document.getElementById('plot-3d').innerHTML = `<h3 style="color:#e74c3c; text-align:center;">Erreur Math</h3>`; }
    }, 100);
};
...
// ==========================================
// 10. SAUVEGARDE ET PROJETS A GAUCHE (UNIFIÉ)
// ==========================================
...
window.saveProject = async () => {
    ...
    try {
        ...
        if(json.status === "success") { 
            ...
            updateDrawUI(); alert("✅ Projet sauvegardé !"); 
        }
...
window.loadProject = async () => {
    ...
    try {
        ...
        data.forEach(d => {
            ...
            const newObj = { 
                id: Date.now()+Math.random(), type:d.type, name:d.name, 
            ...
            newProj.features.push(newObj);
            recalculateStats(newObj); // Calcule l'étiquette
        });
        projectStore.push(newProj); updateProjectUI();
        
        // Ajuster la vue sur le projet chargé
    ...
    projectStore.forEach(p => {
        ...
        p.features.forEach(f => {
            const btn = f.type==='line' ? 
                ...
                `<div style="display:flex; gap:3px; margin-top:5px; flex-wrap:wrap;">
                    <button onclick="calculateVolume(${f.id}, 'hollow')" style="flex:1; font-size:0.75em; background:#2980b9; color:#fff; border:none; cursor:pointer; padding:5px;">📉 Déblai</button>
                    <button onclick="calculateVolume(${f.id}, 'mound')" style="flex:1; font-size:0.75em; background:#e67e22; color:#fff; border:none; cursor:pointer; padding:5px;">📈 Remblai</button>
                    <button onclick="calculateVolume(${f.id}, 'slope')" style="flex:1; font-size:0.75em; background:#8e44ad; color:#fff; border:none; cursor:pointer; padding:5px;">📐 Courbe</button>
                    <button onclick="calculateVolume(${f.id}, 'plane')" style="flex:1; font-size:0.75em; background:#9b59b6; color:#fff; border:none; cursor:pointer; padding:5px;">📏 Plan</button>
                    <button onclick="generate3DView(${f.id})" style="flex:1; min-width:100%; font-size:0.8em; background:#34495e; color:#fff; border:1px solid #555; cursor:pointer; padding:5px; margin-top:2px;">👁️ Lancer Vue 3D</button>
                </div>`;
                
            fHtml += `<div style="margin-left:5px; border-left:3px solid ${f.color}; padding:5px; background:#1a1a1a; margin-top:5px;">
                <div style="display:flex; align-items:center;">
                    <input type="checkbox" checked onchange="toggleProjectFeature(${p.id}, ${f.id})"> 
                    <strong style="margin-left:5px;">${f.name}</strong>
                </div>
